...
// 'game-rt'라는 이름의 데이터 채널 생성
export async function callPeer(peerId) {
  if (!pc) createPeer();

  // ordered:false 순서 섞여도 속도가 최우선, maxRetransmits:0 재전송X -> 반응속도 상승
  ...
  emitSignal(peerId, { type: "offer", sdp: offer.sdp }); // 시그널링
}
